...
interface APICommunity {
    ...
    fun getUser(@Header("Authorization") authToken: String, @Path("username") username: String): Call<UserResponse>

    @GET("/api/v2/friends/{username}/")
    fun getFriendList(@Header("Authorization") authToken: String,@Path("username") username: String): Call<FriendResponse>

    @GET("/api/v2/users/search")
    fun searchUsers(@Header("Authorization") authToken: String,@Query("query") query: String): Call<List<UserResponse>>
    ...
    fun sendRequest(@Header("Authorization") authToken: String,@Path("username") username: String): Call<PostResponse>

    @POST("/api/v2/requests/{username}/accept/")
    fun acceptRequest(@Header("Authorization") authToken: String,@Path("username") username: String): Call<PostResponse>

    @POST("/api/v2/requests/{username}/decline/")
    fun declineRequest(@Header("Authorization") authToken: String,@Path("username") username: String): Call<PostResponse>


    @DELETE("/api/v2/friends/{username}/")
    fun deleteFriend(@Header("Authorization") authToken: String,@Path("username") username: String): Call<PostResponse>
}